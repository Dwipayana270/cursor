(function () {
  ...
  function waitForDivAndInject() {
    const observer = new MutationObserver(() => {
      ...
      if (el && applet && el.childElementCount === 0) {
    ...
    if (elNow && applet && elNow.childElementCount === 0) {
      applet.inject("ggbApplet");
    }
  }
  waitForDivAndInject();
  ...
  function waitForDivAndInject2() {
    const observer = new MutationObserver(() => {
      ...
      if (el && applet2 && el.childElementCount === 0) {
    ...
    if (elNow && applet2 && elNow.childElementCount === 0) {
      applet2.inject("ggbApplet2");
    }
  }
  ...
  function getUIState() {
    try {
      const s = localStorage.getItem("mathUI");
    ...
    if (ui && ui.screen) {
      switch (ui.screen) {
        case "home":
          ...
          break;
        case "placement":
          if (ui.bab) startPlacement(ui.bab);
          else showHome();
          break;
        case "learning":
          if (ui.bab && ui.level) showLearning(ui.bab, ui.level);
          else showHome();
          break;
        case "evaluation":
          if (ui.bab) showEvaluation(ui.bab);
          else showHome();
          break;
        case "progress":
          showProgress();
  ...
  function updateJourneyMap(bab, level) {
    ...
    levels.forEach((lvl, idx) => {
      ...
      if (progress[bab] && progress[bab][lvl])
  ...
  async function loadContent(bab, level) {
    ...
    if (level === "dasar") {
      ...
      cfg = {
        ...
        exercise:
          chapter && chapter.exercise ? chapter.exercise : levelData.exercise,
      };
    }
    ...
    sk2.className = "skeleton skeleton-line";
    const sk3 = document.createElement("div");
    sk3.className = "skeleton skeleton-line";
    skeletonWrap.appendChild(sk1);
    skeletonWrap.appendChild(sk2);
    skeletonWrap.appendChild(sk3);
    ...
    try {
      ...
      if (el1 && applet && el1.childElementCount === 0) {
        applet.inject("ggbApplet");
      }
      const el2 = document.getElementById("ggbApplet2");
      if (el2 && applet2 && el2.childElementCount === 0) {
        applet2.inject("ggbApplet2");
      }
    } catch (_) {}
  ...
  function checkExercise() {
    ...
    const answerIndex = parseInt(selected.value);
    let exercise;
    if (currentLevel === "dasar")
      exercise = window.contentData.dasar[currentBab].exercise;
    else {
    ...
    if (exercise.options[answerIndex].correct) {
      ...
      p.textContent = "Mantap! Jawaban kamu benar 🎉";
      ...
      const p = document.createElement("p");
      p.className = "font-bold text-lg";
      p.textContent = "Masih belum tepat. Coba lagi ya! 🤔";
      resultDiv.appendChild(p);
    }
  ...
  function previousLevel() {
    ...
    if (idx > 0) showLearning(currentBab, levels[idx - 1]);
    else showHome();
    scrollToTop();
  }
  ...
  function nextLevel() {
    const levels = ["dasar", "menengah", "lanjut"];
    const idx = levels.indexOf(currentLevel);
    if (idx < levels.length - 1) showLearning(currentBab, levels[idx + 1]);
  ...
  function submitEvaluation() {
    ...
    if (percentage >= 80) {
      ...
      resultText = "Hebat! Kamu menguasai materi ini dengan sangat baik.";
      resultEmoji = "🎉👏";
    } else if (percentage >= 60) {
      resultClass = "neo-brutal-yellow rounded-brutal p-6";
      resultText = "Keren! Kamu sudah cukup paham materinya.";
      resultEmoji = "👍😊";
    } else {
      resultClass = "neo-brutal-red rounded-brutal p-6 text-white";
      resultText = "Ayo semangat! Kamu perlu sedikit lagi memahami materi ini.";
      resultEmoji = "📚💪";
    }

    resultDiv.className = resultClass;
    const emoji = document.createElement("div");
    emoji.className = "text-4xl mb-4";
    emoji.textContent = resultEmoji;
    const h3 = document.createElement("h3");
    h3.className = "text-2xl font-bold mb-2";
    h3.textContent = "HASIL EVALUASI";
    const p1 = document.createElement("p");
    p1.className = "text-xl mb-2";
    p1.textContent = `Skor: ${score}/${questions.length} (${percentage}%)`;
    const p2 = document.createElement("p");
    p2.className = "text-lg";
    p2.textContent = resultText;
    const actions = document.createElement("div");
    actions.className = "mt-6 flex justify-center space-x-4";
    const homeBtn = document.createElement("button");
    homeBtn.className = "btn-brutal neo-brutal-yellow text-black px-6 py-3";
    homeBtn.textContent = "Kembali ke Beranda";
    ...
    const progress = getProgress();
    if (!progress[currentBab]) progress[currentBab] = {};
    progress[currentBab].evaluation = {
      score,
  ...
  function saveProgressData(data) {
    localStorage.setItem("mathProgress", JSON.stringify(data));
  }
  ...
  function updateProgressUI(progress) {
    const renderBabProgress = (containerId, key) => {
      ...
      const levelP = document.createElement("div");
      levelP.className = "mb-2";
      const levelSpan = document.createElement("span");
      levelSpan.className = "font-bold";
      levelSpan.textContent = `Level: ${
        progress[key].level || "Belum ditentukan"
      }`;
      ...
      const mkRow = (labelText, done) => {
        ...
        barWrap.className = "flex-1 h-4 bg-gray-300 rounded";
      ...
      if (
        progress[key].evaluation &&
        typeof progress[key].evaluation.score !== "undefined"
      ) {
        const evalBox = document.createElement("div");
        evalBox.className =
          "mt-4 p-3 bg-blue-100 border-2 border-black rounded text-green-700";
        const span = document.createElement("span");
        span.className = "font-bold";
        span.textContent = `Evaluasi: ${progress[key].evaluation.score}/${progress[key].evaluation.total} (${progress[key].evaluation.percentage}%)`;
        evalBox.appendChild(span);
        container.appendChild(evalBox);
      }
    };
  ...
  function resetProgress() {
    ...
    successDiv.className =
      "fixed top-4 right-4 neo-brutal-green rounded-brutal p-4 text-white z-50";
    const p = document.createElement("p");
    p.className = "font-bold text-lg";
    p.textContent = "✓ Progress berhasil direset!";
    ...
    updateProgress();
    if (
      !document.getElementById("progress-screen").classList.contains("hidden")
    )
      updateProgressUI({});
  }
  ...
  function checkAnswer(button, correctAnswer, babId) {
    ...
    if (allButtons.length === answeredButtons.length) {
      ...
      const correctCount = container.querySelectorAll(
        ".answer-btn.border-green-500"
      ).length;
      if (resultDiv) {
        resultDiv.classList.remove("hidden");
        resultDiv.className =
          "mt-4 text-center font-bold text-lg " +
          (correctCount === allButtons.length
            ? "text-green-600"
            : "text-yellow-600");
        resultDiv.textContent =
          correctCount === allButtons.length
            ? "🎉 Semua benar! Kamu siap ke Level Menengah!"
            : `Benar ${correctCount}/${allButtons.length}. Terus berlatih!`;
      }
    }
  }
  ...
  async function checkEssayAnswer(button, babId, questionId) {
    ...
    if (!userAnswer) {
      alert("Silakan isi jawaban dulu ya!");
      return;
    }
